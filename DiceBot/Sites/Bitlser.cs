...
{
    internal class Bitsler : DiceSite
    {
        public static string[] sCurrencies =
        {
            "BTC", "ETH", "LTC", "BCH", "XRP", "DOGE", "DASH", "BSV", "ZEC", "ETC", "NEO", "KMD", "BTG", "LSK", "DGB", "QTUM", "STRAT", "WAVES", "BURST",
            "BTSLR", "BSV", "XLM", "USDT"
        };

        private readonly DateTime LastReset = new DateTime();
            ...
            while (IsBitsler)
            {
                if ((DateTime.Now - lastupdate).TotalSeconds > 60 || ForceUpdateStats)
                {
                    ...
                    try
                    {
                        ...
                        var resp = Client.PostAsync("api/getuserstats", Content).Result;

                        var s1 = "";
                        var sEmitResponse = ""; // resp.Content.ReadAsStringAsync().Result;
                        ...
                        if (sEmitResponse != "")
                        {
                            var bsstatsbase = JsonUtils.JsonDeserialize<bsStats>(sEmitResponse.Replace("\"return\":", "\"_return\":"));
                            ...
                            if (bsstatsbase != null)
                                if (bsstatsbase != null)
                                    if (bsstatsbase.success == "true")
                                    {
                                        switch (Currency.ToLower())
                                        {
                                            case "btc":
                                                ...
                                                wagered = bsstatsbase.btc_wagered;

                                                break;
                                            case "ltc":
                                                balance = bsstatsbase.ltc_balance;
                                                profit = bsstatsbase.ltc_profit;
                                                wagered = bsstatsbase.ltc_wagered;
                                        ...
                                        bets = int.Parse(bsstatsbase.bets, NumberFormatInfo.InvariantInfo);
                                        wins = int.Parse(bsstatsbase.wins, NumberFormatInfo.InvariantInfo);
                                        losses = int.Parse(bsstatsbase.losses, NumberFormatInfo.InvariantInfo);
                                    ...
                                    {
                                        if (bsstatsbase.error != null) Parent.updateStatus(bsstatsbase.error);
                                    }
                        }
                    }
                    catch
        ...
        protected override void CurrencyChanged()
        {
            ...
            if (accesstoken != "" && IsBitsler)
                try
                {
                    ...
                    pairs.Add(new KeyValuePair<string, string>("access_token", accesstoken));
                    var Content = new FormUrlEncodedContent(pairs);
                    var sEmitResponse = Client.PostAsync("api/getuserstats", Content).Result.Content.ReadAsStringAsync().Result;
                    var bsstatsbase = JsonUtils.JsonDeserialize<bsStats>(sEmitResponse.Replace("\"return\":", "\"_return\":"));
...
currency:btc*/
                pairs.Add(new KeyValuePair<string, string>("access_token", accesstoken));
                pairs.Add(new KeyValuePair<string, string>("amount", tmpob.Amount.ToString("0.00000000", NumberFormatInfo.InvariantInfo)));
                pairs.Add(new KeyValuePair<string, string>("over", tmpob.High.ToString().ToLower()));

                pairs.Add(new KeyValuePair<string, string>("target",
                                                           !tmpob.High
                                                               ? tmpob.Chance.ToString("0.00", NumberFormatInfo.InvariantInfo)
                                                               : (maxRoll - tmpob.Chance).ToString("0.00", NumberFormatInfo.InvariantInfo)));
                ...
                var Content = new FormUrlEncodedContent(pairs);
                var tmpmsg = Client.PostAsync("api/bet-dice", Content).Result;
                var sEmitResponse = tmpmsg.Content.ReadAsStringAsync().Result;
                bsBet bsbase = null;

                try
                {
                    bsbase = JsonUtils.JsonDeserialize<bsBet>(sEmitResponse.Replace("\"return\":", "\"_return\":"));
                }
                catch (Exception e)
                {
                }
                ...
                if (bsbase != null)
                    if (bsbase != null)
                        if (bsbase.success)
                        {
                            balance = decimal.Parse(bsbase.new_balance, NumberFormatInfo.InvariantInfo);
                            lastupdate = DateTime.Now;
                            var tmp = bsbase.ToBet();
                            tmp.Guid = tmpob.Guid;
                            profit += tmp.Profit;
                            wagered += tmp.Amount;
                            tmp.date = DateTime.Now;
                            var win = false;
                            if (tmp.Roll > 99.99m - tmp.Chance && tmp.high || tmp.Roll < tmp.Chance && !tmp.high) win = true;
        ...
        {
            var tBetThread = new Thread(PlaceBetThread);
        ...
        public override void ResetSeed()
        {
            ...
            try
            {
                if ((DateTime.Now - LastReset).TotalMinutes >= 3)
                {
                    ...
                    var Content = new FormUrlEncodedContent(pairs);
                    var sEmitResponse = Client.PostAsync("api/change-seeds", Content).Result.Content.ReadAsStringAsync().Result;
                    var bsbase = JsonUtils.JsonDeserialize<bsResetSeed>(sEmitResponse.Replace("\"return\":", "\"_return\":"));

                    //sqlite_helper.InsertSeed(bsbase.last_seeds_revealed.seed_server_hashed, bsbase.last_seeds_revealed.seed_server_revealed);
                    SQLiteHelper.InsertSeed(bsbase.previous_hash, bsbase.previous_seed);
                }
                else
                {
        ...
        public override void Login(string Username, string Password, string twofa)
        {
            ...
            try
            {
                ...
                if (twofa.Contains("&"))
                {
                    var pars = twofa.Split('&');
                    actual2fa = pars[1];
                    twofa = pars[0];
                }

                var resp = Client.GetAsync("https://www.bitsler.com").Result;
                var s1 = "";
                ...
                //pairs.Add(new KeyValuePair<string, string>("api_key", "0b2edbfe44e98df79665e52896c22987445683e78"));
                if (!string.IsNullOrWhiteSpace(actual2fa)) pairs.Add(new KeyValuePair<string, string>("two_factor", actual2fa));
                pairs.Add(new KeyValuePair<string, string>("api_key", twofa));
                var Content = new FormUrlEncodedContent(pairs);
                var tmpresp = Client.PostAsync("api/login", Content).Result;

                var bytes = tmpresp.Content.ReadAsByteArrayAsync().Result;
                var sEmitResponse = tmpresp.Content.ReadAsStringAsync().Result;

                //getuserstats 
                var bsbase = JsonUtils.JsonDeserialize<bsLogin>(sEmitResponse.Replace("\"return\":", "\"_return\":"));
                            ...
                            if (bsstatsbase != null)
                                if (bsstatsbase != null)
                                    if (bsstatsbase.success == "true")
                                    {
                                        ...
                                        bets = int.Parse(bsstatsbase.bets == null ? "0" : bsstatsbase.bets, NumberFormatInfo.InvariantInfo);
                                        wins = int.Parse(bsstatsbase.wins == null ? "0" : bsstatsbase.wins, NumberFormatInfo.InvariantInfo);
                                        losses = int.Parse(bsstatsbase.losses == null ? "0" : bsstatsbase.losses, NumberFormatInfo.InvariantInfo);
    ...
    public class bsBet
    {
        ...
        public Bet ToBet()
        {
            var tmp = new Bet
            {
                Amount = decimal.Parse(amount, NumberFormatInfo.InvariantInfo),
                date = DateTime.Now,
                Id = id,
