...
{
    internal class Dice999 : DiceSite
    {
        public static string[] cCurrencies = {"btc", "doge", "ltc", "eth", "xmr"};
        private HttpClient Client; // = new HttpClient { BaseAddress = new Uri("https://www.999dice.com/api/web.aspx") };
        ...
        private void GetBalanceThread()
        {
            while (isD999)
            {
                if (sessionCookie != "" && sessionCookie != null && ((DateTime.Now - Lastbalance).TotalSeconds >= 60 || ForceUpdateStats)) GetBalance();
                Thread.Sleep(1100);
            }
        }
        ...
        private void GetBalance()
        {
            if (sessionCookie != "" && sessionCookie != null && (DateTime.Now - Lastbalance).TotalSeconds > 60)
            {
                ...
                try
                {
                    var tmplogin = JsonUtils.JsonDeserialize<d999Login>(responseData);
                    balance = tmplogin.Balance / 100000000.0m;
                    wagered = Wagered = -tmplogin.TotalPayIn / 100000000.0m;
        ...
        private void PlaceBetThread(object _High)
        {
            ...
            try
            {
                ...
                Parent.updateStatus(string.Format(NumberFormatInfo.InvariantInfo, "Betting: {0:0.00000000} at {1:0.00000000} {2}", amount, tmp9.Chance,
                                                  High ? "High" : "Low"));

                var chance = 999999.0m * (tmp9.Chance / 100.0m);
                ...
                if (next == "" && next != null)
                {
                    ...
                    var Hash = next = JsonUtils.JsonDeserialize<d999Hash>(responseData).Hash;
                }
                ...
                pairs.Add(new KeyValuePair<string, string>("s", sessionCookie));
                pairs.Add(new KeyValuePair<string, string>("PayIn", ((long) (amount * 100000000m)).ToString("0", NumberFormatInfo.InvariantInfo)));
                pairs.Add(new KeyValuePair<string, string>("Low", (High ? 999999 - (int) chance : 0).ToString(NumberFormatInfo.InvariantInfo)));
                pairs.Add(new KeyValuePair<string, string>("High", (High ? 999999 : (int) chance).ToString(NumberFormatInfo.InvariantInfo)));
                pairs.Add(new KeyValuePair<string, string>("ClientSeed", ClientSeed));
                pairs.Add(new KeyValuePair<string, string>("Currency", Currency));
                pairs.Add(new KeyValuePair<string, string>("ProtocolVersion", "2"));

                Content = new FormUrlEncodedContent(pairs);
                var tmps = Content.ReadAsStringAsync().Result;
                ...
                var tmpBet = JsonUtils.JsonDeserialize<d999Bet>(responseData);

                if (amount >= 21)
                {
                }

                if (tmpBet.ChanceTooHigh == 1 || (tmpBet.ChanceTooLow == 1) | (tmpBet.InsufficientFunds == 1) || tmpBet.MaxPayoutExceeded == 1 ||
                    tmpBet.NoPossibleProfit == 1)
                {
                ...
                if (tmpBet.BetId == 0) throw new Exception();

                balance = tmpBet.StartingBalance / 100000000.0m - amount + tmpBet.PayOut / 100000000.0m;

                profit += -amount + tmpBet.PayOut / 100000000m;
                var tmp = new Bet();
                tmp.Guid = tmp9.Guid;
                tmp.Amount = amount;
                ...
                tmp.nonce = 0;
                tmp.Profit = tmpBet.PayOut / 100000000m - amount;
                ...
                var win = false;
                if (tmp.Roll > 99.99m - tmp.Chance && High || tmp.Roll < tmp.Chance && !High) win = true;
        ...
        public override bool InternalSendTip(string User, decimal amount)
        {
            ...
            pairs.Add(new KeyValuePair<string, string>("s", sessionCookie));
            pairs.Add(new KeyValuePair<string, string>("Currency", Currency));
            pairs.Add(new KeyValuePair<string, string>("Amount", (amount * 100000000m).ToString("0", NumberFormatInfo.InvariantInfo)));
            ...
            using (var response = Client.PostAsync("", Content))
            {
                ...
                {
                    if (e.InnerException.Message.Contains("ssl")) return InternalSendTip(User, amount);
                }
            }
        ...
        protected override bool internalWithdraw(decimal Amount, string Address)
        {
            ...
            using (var response = Client.PostAsync("", Content))
            {
                ...
                {
                    if (e.InnerException.Message.Contains("ssl")) return internalWithdraw(Amount, Address);
                }
            }
        ...
        public override void Login(string Username, string Password, string twofa)
        {
            try
            {
                ...
                var tmpU = JsonUtils.JsonDeserialize<d999Login>(responseData);
        ...
        public override bool Register(string username, string password)
        {
            ...
            using (var response = Client.PostAsync("", Content))
            {
                ...
                {
                    if (e.InnerException.Message.Contains("ssl")) return Register(username, password);
                }
            }

            var tmp = JsonUtils.JsonDeserialize<d999Register>(responseData);
        ...
        public void GetDepositAddress()
        {
            if (sessionCookie != "" && sessionCookie != null)
            {
                ...
                var tmp = JsonUtils.JsonDeserialize<d999deposit>(responseData);
        ...
        public static decimal sGetLucky(string serverSeed, string clientSeed, long betNumber /*, long betResult*/, string serverSeedHash = null)
        {
            ...
            var server = strtobytes(serverSeed);
            var client = BitConverter.GetBytes(int.Parse(clientSeed)).Reverse().ToArray();
            var num = BitConverter.GetBytes(betNumber).Reverse().ToArray();
            var data = server.Concat(client).Concat(num).ToArray();
            ...
            using (SHA512 sha512 = new SHA512Managed())
            {
                ...
                var hash = sha512.ComputeHash(sha512.ComputeHash(data));
                ...
                while (true)
                {
                    for (var x = 0; x <= 61; x += 3)
                    {
                        ...
                        if (result < 16000000) return result % 1000000m / 10000.0m;
                    }
    ...
    public class d999Login : d999Register
    {
        ...
        public long BetLoseCount => BetCount - BetWinCount;
        public decimal BetPayIn { get; set; }
        public decimal BetPayOut { get; set; }
        public decimal Profit => BetPayIn + BetPayOut;
        public decimal Wagered => BetPayOut - BetPayIn;

        public decimal TotalPayIn { get; set; }
        public decimal TotalPayOut { get; set; }
        public decimal TotalProfit => TotalPayIn + TotalPayOut;

        public long TotalBets { get; set; }
        public long TotalWins { get; set; }
        public long TotalLoseCount => TotalBets - TotalWins;
    }
