...
{
    internal class BitDice : DiceSite
    {
        public static string[] cCurrencies = {"btc", "doge", "ltc", "eth", "csno", "eos"};
        private readonly Random R = new Random();
        private string APIKey = "";
        private HttpClient Client;
        private HttpClientHandler ClientHandlr;
        private BDSeed CurrentSeed;
        private bool isbitdice;
        ...
        public BitDice(cDiceBot Parent)
        {
            ...
            AutoUpdate = false;
            Currencies = new[] {"btc", "doge", "ltc", "eth", "csno", "eos"};
            Currency = "BTC";
        }
        ...
        public override void Login(string Username, string Password, string twofa)
        {
            ...
            try
            {
                var Response = Client.GetStringAsync($"user/balance?api_key={APIKey}&currency={Currency}").Result;
                var tmpStats = JsonUtils.JsonDeserialize<BDSTats>(Response);

                //Parent.DumpLog(Response, -1);
                var SecretResponse = Client.GetStringAsync($"dice/secret?api_key={APIKey}").Result;
                var tmpSeed = JsonUtils.JsonDeserialize<BDSeed>(SecretResponse);

                //Parent.DumpLog(SecretResponse, -1);
                balance = tmpStats.balance;
                wagered = tmpStats.wagered;
                profit = tmpStats.profit;
        ...
        public void PlaceBetThread(object BetObject)
        {
            try
            {
                ...
                var Highlow = curBet.High ? "high" : "low";

                var request = string.Format(NumberFormatInfo.InvariantInfo, "dice?api_key={0}&currency={1}&amount={2}&chance={3}&type={4}&client={5}",
                                            APIKey,
                                            Currency,
                                            curBet.Amount,
                                            curBet.Chance,
                ...
                var sbetresult = BetResponse.Content.ReadAsStringAsync().Result;

                var NewBet = JsonUtils.JsonDeserialize<BDBetResponse>(sbetresult);
                ...
                try
                {
                    ...
                    var win = NewBet.bet.data.result;
