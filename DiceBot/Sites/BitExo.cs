...
{
    internal class BitExo : DiceSite
    {
        public static string[] sCurrencies = {"BTC", "BXO", "CLAM", "DOGE", "LTC", "ETH"};
        private readonly Random R = new Random();
        private readonly Dictionary<long, BitExoReqType> Requests = new Dictionary<long, BitExoReqType>();
        private readonly string url = "bit-exo.com";
        ...
        private WebSocket WSClient; // = new WebSocket("");
        ...
        public override void Login(string Username, string Password, string twofa)
        {
            ...
            {
                UseCookies = true, CookieContainer = cookies, AutomaticDecompression = DecompressionMethods.Deflate | DecompressionMethods.GZip, Proxy = Prox,
                UseProxy = Prox != null
            };
        ...
        private void ConnectSocket()
        {
            ...
            var response = Client.GetStringAsync("socket.io/?EIO=3&transport=polling&t=" + JsonUtils.CurrentDate()).Result;
            ...
            var chatinit = "42" + id++ + "[\"access_token_data\",{\"access_token\":\"" + accesstoken + "\"}]";
            chatinit = chatinit.Length + ":" + chatinit;
            var content = new StringContent(chatinit, Encoding.UTF8, "application/octet-stream");

            response = Client.PostAsync("socket.io/?EIO=3&transport=polling&t=" + JsonUtils.CurrentDate() + "&sid=" + sid, content).Result.Content
            ...
            Headers.Add(new KeyValuePair<string, string>("User-Agent",
                                                         "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.87 Safari/537.36"));
            ...
            while (ispd)
            {
                try
                {
                    if (WSClient.State == WebSocketState.Open && ((DateTime.Now - lastupdate).TotalSeconds > 15 || ForceUpdateStats))
                    {
                        ...
                        WSClient.Send("2");
                        var tmpid = id++;
                        Requests.Add(tmpid, BitExoReqType.balance);
        ...
        {
            var cl = long.Parse(client);
            var serverseed = decimal.Parse(server.Substring(0, server.IndexOf("-")), NumberFormatInfo.InvariantInfo);
            var rollb = (serverseed + cl) % 4294967296;
            var roll = rollb / (decimal) Math.Pow(2, 32) * 99.9999m;
            roll = roll * 10000;
        ...
        private void WSClient_MessageReceived(object sender, MessageReceivedEventArgs e)
        {
            ...
            if (e.Message == "3probe")
            {
                WSClient.Send("5");
                var tmpid = id++;
                Requests.Add(tmpid, BitExoReqType.hash);
            ...
            else
            {
                try
                {
                    ...
                    if (long.TryParse(id, out lid))
                        ...
                        {
                            var tmp = Requests[lid];
                            Requests.Remove(lid);

                            switch (tmp)
                            {
                                case BitExoReqType.balance:
                                    response = response.Substring(response.IndexOf("{"));
                                    response = response.Substring(0, response.LastIndexOf("}") + 1);
                                    ProcessBalance(response);

                                    break;
                                case BitExoReqType.hash:
                                    response = response.Substring(response.IndexOf("\"") + 1);
                                    response = response.Substring(0, response.LastIndexOf("\""));
                                    ProcessHash(response);

                                    break;
                                case BitExoReqType.bet:
        ...
        {
            var tmpid = id++;
            Requests.Add(tmpid, BitExoReqType.tip);

            //426["send_tip",{"uname":"professor","amount":1000,"private":false,"type":"BTC"}]
            var request = string.Format("42{3}[\"send_tip\",{{\"uname\":\"{0}\",\"amount\":{1},\"private\":false,\"type\":\"{2}\"}}]",
                                        User,
        ...
        private void ProcessBalance(string Res)
        {
            var tmpResult = JsonUtils.JsonDeserialize<BEBalanceBase>(Res);

            switch (Currency.ToLower())
            {
                case "btc":
                    balance = tmpResult.user.balances.btc / 100000000m;
                    bets = (int) tmpResult.user.stats.btc.bets;
                    wagered = tmpResult.user.stats.btc.wager / 100000000m;
                    profit = tmpResult.user.stats.btc.profit / 100000000m;

                    break;
                case "bxo":
                    balance = tmpResult.user.balances.bxo / 100000000m;
                    bets = (int) tmpResult.user.stats.bxo.bets;
                    wagered = tmpResult.user.stats.bxo.wager / 100000000m;
                    profit = tmpResult.user.stats.bxo.profit / 100000000m;

                    break;
                case "clam":
                    balance = tmpResult.user.balances.clam / 100000000m;
                    bets = (int) tmpResult.user.stats.clam.bets;
                    wagered = tmpResult.user.stats.clam.wager / 100000000m;
                    profit = tmpResult.user.stats.clam.profit / 100000000m;

                    break;
                case "doge":
                    balance = tmpResult.user.balances.doge / 100000000m;
                    bets = (int) tmpResult.user.stats.doge.bets;
                    wagered = tmpResult.user.stats.doge.wager / 100000000m;
                    profit = tmpResult.user.stats.doge.profit / 100000000m;

                    break;
                case "eth":
                    balance = tmpResult.user.balances.eth / 100000000m;
                    bets = (int) tmpResult.user.stats.eth.bets;
                    wagered = tmpResult.user.stats.eth.wager / 100000000m;
                    profit = tmpResult.user.stats.eth.profit / 100000000m;

                    break;
                case "flash":
                    balance = tmpResult.user.balances.flash / 100000000m;
                    bets = (int) tmpResult.user.stats.flash.bets;
                    wagered = tmpResult.user.stats.flash.wager / 100000000m;
                    profit = tmpResult.user.stats.flash.profit / 100000000m;

                    break;
                case "ltc":
                    balance = tmpResult.user.balances.ltc / 100000000m;
                    bets = (int) tmpResult.user.stats.ltc.bets;
                    wagered = tmpResult.user.stats.ltc.wager / 100000000m;
                    profit = tmpResult.user.stats.ltc.profit / 100000000m;
        ...
        private void ProcessHash(string Res)
        {
            ServerHash = Res;
        }

        private void ProcessBet(string Res)
        {
            ...
            var tmpResult = JsonUtils.JsonDeserialize<BEBetResult>(Res);

            var newBet = new Bet
            {
                Amount = tmpResult.wager / 100000000m,
                date = DateTime.Now,
                ...
                Roll = tmpResult.outcome,
                serverhash = ServerHash,
                serverseed = tmpResult.secret + "-" + tmpResult.salt,
                Profit = tmpResult.profit / 100000000m,
                Chance = chance
            };
            ...
            ServerHash = tmpResult.next_hash;
            FinishedBet(newBet);
        }
        ...
        protected override void internalPlaceBet(bool High, decimal amount, decimal chancem, string BetGuid)
        {
            try
            {
                ...
                this.High = High;
                chance = chancem;
                Requests.Add(tmpid, BitExoReqType.bet);

                var request =
                    string.Format("42{7}[\"dice_bet\",{{\"wager\":{0:0},\"client_seed\":{1},\"hash\":\"{2}\",\"cond\":\"{3}\",\"target\":{4},\"payout\":{5},\"currency\":\"{6}\"}}]",
                                  Math.Floor(amount * 100000000m),
                                  clientseed,
                                  ServerHash,
                                  High ? ">" : "<",
