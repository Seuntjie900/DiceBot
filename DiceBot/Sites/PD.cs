...
namespace DiceBot.Sites
{
    ...
    {
        public static string[] sCurrencies = {"Btc", "Ltc", "Eth", "Doge", "Bch", "XRP", "TRX"};
        private readonly bool getid;

        private readonly Random R = new Random();
        private readonly long uid = 0;
            ...
            try
            {
                while (ispd)
                {
                    if (userid != null && ((DateTime.Now - lastupdate).TotalSeconds >= 30 || ForceUpdateStats))
                    {
                        ...
                        {
                            Query =
                                "query{user {activeServerSeed { seedHash seed nonce} activeClientSeed{seed} id balances{available{currency amount}} statistic {game bets wins losses betAmount profit currency}}}"
                        };

                        var Resp = GQLClient.PostAsync(LoginReq).Result;
                        var user = Resp.GetDataFieldAs<pdUser>("user");

                        foreach (var x in user.statistic)
                        {
                            if (x.currency.ToLower() == Currency.ToLower() && x.game == StatGameName)
                            {
                                ...
                                wagered = (decimal) x.betAmount;

                                break;
                            }
                        }

                        foreach (var x in user.balances)
                        {
        ...
        private void placebetthread(object bet)
        {
            try
            {
                ...
                var betresult = GQLClient.PostAsync(new GraphQLRequest
                {
                    Query = "mutation{" + RolName + "(amount:" + amount.ToString("0.00000000", NumberFormatInfo.InvariantInfo) + ", target:" +
                            tmpchance.ToString("0.00", NumberFormatInfo.InvariantInfo) + ",condition:" + (High ? "above" : "below") + ",currency:" +
                            Currency.ToLower() + ") { id nonce currency amount payout state { ... on " + GameName +
                            " { result target condition } } createdAt serverSeed{seedHash seed nonce} clientSeed{seed} user{balances{available{amount currency}} statistic{game bets wins losses betAmount profit currency}}}}"
                }).Result;
                ...
                {
                    var tmp = betresult.GetDataFieldAs<RollDice>(RolName);
                    ...
                    try
                    {
                        ...
                        foreach (var x in tmp.user.statistic)
                        {
                            if (x.currency.ToLower() == Currency.ToLower() && x.game == StatGameName)
                            {
                                bets = (int) x.bets;
                        ...
                        var tmpbet = tmp.ToBet(maxRoll);

                        if (getid)
                        {
                            var betresult2 = GQLClient.PostAsync(new GraphQLRequest {Query = " query{bet(betId:\"" + tmpbet.Id + "\"){iid}}"}).Result;
        ...
        public override void ResetSeed()
        {
            try
            {
                var LoginReq = new GraphQLRequest
                {
                    Query = "mutation{rotateServerSeed{ seed seedHash nonce } changeClientSeed(seed:\"" + R.Next(0, int.MaxValue) + "\"){seed}}"
                };
                ...
                if (Resp.Data != null)
                {
                    var user = Resp.GetDataFieldAs<pdSeed>("rotateServerSeed");
                }
        ...
        protected override bool internalWithdraw(decimal Amount, string Address)
        {
            try
            {
                var LoginReq = new GraphQLRequest
                {
                    Query = "mutation{createWithdrawal(currency:" + Currency.ToLower() + ", address:\"" + Address + "\",amount:" +
                            amount.ToString("0.00000000", NumberFormatInfo.InvariantInfo) +
                            "){id name address hash amount walletFee createdAt status currency}}"
                };
        ...
        public override bool InternalSendTip(string User, decimal amount)
        {
            try
            {
                ...
                var userid = GetUid(User);

                var LoginReq = new GraphQLRequest
                {
                    Query = "mutation{sendTip(userId:\"" + userid + "\", amount:" + amount.ToString("0.00000000", NumberFormatInfo.InvariantInfo) +
                            ", currency:" + Currency.ToLower() + ",isPublic:true,chatId:\"14939265-52a4-404e-8a0c-6e3e10d915b4\"){id currency amount}}"
                };
        ...
        private void Send(object _Message)
        {
            if (accesstoken != "")
                try
                {
                    ...
                    var sEmitResponse = Client.PostAsync("send?api_key=" + accesstoken, Content).Result.Content.ReadAsStringAsync().Result;
                }
                catch
                {
                }
