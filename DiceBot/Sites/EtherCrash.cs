    ...
    internal class EtherCrash : DiceSite
    {
        ...
        public override void Login(string Username, string Password, string twofa)
        {
            ...
            try
            {
                ...
                var Response = Client.GetStringAsync("https://www.ethercrash.io/play").Result;

                Response = Client.GetStringAsync("https://www.ethercrash.io/socket.io/?EIO=3&transport=polling&t=" + JsonUtils.CurrentDate()).Result;

                Response = Client.GetStringAsync("https://gs.ethercrash.io/socket.io/?EIO=3&transport=polling&t=" + JsonUtils.CurrentDate()).Result;
                ...
                Response = Client.GetStringAsync("https://www.ethercrash.io/socket.io/?EIO=3&sid=" + iochat + "&transport=polling&t=" + JsonUtils.CurrentDate())
                    .Result;
                ...
                var ottcontent = new StringContent("");
                var RespMsg = Client.PostAsync("https://www.ethercrash.io/ott", ottcontent).Result;
                ...
                var stringContent = new StringContent(body, Encoding.UTF8, "text/plain");

                Response = Client
                    .PostAsync("https://gs.ethercrash.io/socket.io/?EIO=3&sid=" + io + "&transport=polling&t=" + JsonUtils.CurrentDate(), stringContent).Result
                    .Content.ReadAsStringAsync().Result;

                body = "420[\"join\",[\"english\"]]";
                body = body.Length + ":" + body;
                var stringContent2 = new StringContent(body, Encoding.UTF8, "text/plain");

                Response = Client
                    .PostAsync("https://www.ethercrash.io/socket.io/?EIO=3&sid=" + iochat + "&transport=polling&t=" + JsonUtils.CurrentDate(), stringContent2)
        ...
        private void Sock_MessageReceived(object sender, MessageReceivedEventArgs e)
        {
            ...
            else
            {
                ...
                else if (e.Message.StartsWith("430[null,{\"state"))
                {
                    ...
                    var tmplogin = JsonUtils.JsonDeserialize<ECLogin>(content);

                    if (tmplogin != null)
                    {
                        username = tmplogin.username;
                        balance = tmplogin.balance_satoshis / 100000000m;
        ...
        private void Sock_Error(object sender, ErrorEventArgs e)
        {
        ...
        protected override void internalPlaceBet(bool High, decimal amount, decimal chancem, string BetGuid)
        {
            if (ReadyToBet() && Sock.State == WebSocketState.Open)
            {
                ...
                amount = this.amount;
                var payout = (100 - edge) / chance;
                var returna = payout * 100;
                Sock.Send("42" + reqid++ + "[\"place_bet\"," + (amount * 100000000).ToString("0") + "," + returna.ToString("0") + "]");
                guid = BetGuid;
