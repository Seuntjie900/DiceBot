...
namespace DiceBot.Forms
{
    ...
    public partial class cDiceBot : Form
    {
        ...
        public void GetBetResult(decimal Balance, Bet bet)
        {
            try
            {
                DumpLog("received bet result: Balance: " + Balance + ", Bet:" + JsonUtils.JsonSerializer<Bet>(bet), 8);
                ...
                if (LogLevel > 2)
                    ...
                    {
                        sw.WriteLine(JsonUtils.JsonSerializer<Bet>(bet));
                    }
            }
            catch (Exception e)
            {
...
function dobet()
        ...
        */
        ...
        {
            return SQLiteHelper.GetHistoryByQuery(Query);
        }
        ...
        Bet[] luagethistory()
        {
            return SQLiteHelper.GetBetHistory(CurrentSite.Name);
        }
        ...
        Bet[] luagethistory(string From, string untill)
        {
            try
            {
                return SQLiteHelper.GetBetHistory(CurrentSite.Name, DateTime.Parse(From, System.Globalization.DateTimeFormatInfo.InvariantInfo), DateTime.Parse(untill, System.Globalization.DateTimeFormatInfo.InvariantInfo));
            }
            catch (Exception e)
            {
        ...
        private void Tmp_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (!startupMessage)
            {
                Settings tmpSet = new Settings(this);
    ...
    void setInterval(System.Windows.Forms.Timer tmr, int Interval)
    {
        ...
        private void btnChartBetID_Click(object sender, EventArgs e)
        {
            ...
            if (tmp.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                if (tmp.ChartType == 1)
                {
                    Graph g = new Graph(SQLiteHelper.GetBetForCharts(CurrentSite.Name, tmp.StartID));
                    g.Show();
                }
                else
                {
                    Graph g = new Graph(SQLiteHelper.GetBetForCharts(CurrentSite.Name, tmp.StartDate , tmp.EndDate));
                    g.Show();
                }
            }
        ...
        private void btnGraphProfitBets_Click(object sender, EventArgs e)
        {
            ...
            if (LiveGraph != null)
            {
                ...
                {
                    LiveGraph = new Graph(SQLiteHelper.GetBetForCharts(CurrentSite.Name, OpenTime, DateTime.Now.AddYears(1)));
                    LiveGraph.Show();
                    created = true;
                }
            }
        ...
        {
            Graph g = new Graph(SQLiteHelper.GetBetForCharts(CurrentSite.Name, OpenTime, OpenTime.AddYears(1)));
            g.Show();
        }
        ...
        private void btnChartAllTimeProfitBets_Click(object sender, EventArgs e)
        {
            ...
            if (LiveGraph != null)
            {
                if (LiveGraph.IsDisposed)
                {
                    LiveGraph = new Graph(SQLiteHelper.GetBetForCharts(CurrentSite.Name));
                    LiveGraph.Show();
                    created = true;
                }
            }
        ...
        {
            Graph g = new Graph(SQLiteHelper.GetBetForCharts(CurrentSite.Name));
            g.Show();
        }
        ...
        public void AddBet(object Bet)
        {
            ...
            else
            {
                ...
                SQLiteHelper.AddBet(Bet as Bet, CurrentSite.Name);
                dataGridView1.DataBindings.Clear();
                Bet _Bet = (Bet as Bet);
                if (logging > 2)
                using (StreamWriter sw = File.AppendText("log.txt"))
                {
                    sw.WriteLine(JsonUtils.JsonSerializer<Bet>(_Bet));
                }
        ...
        void GetMissingSeeds()
        {
            ...
            BetIDs = SQLiteHelper.GetMissingSeedIDs(CurrentSite.Name);
        }
