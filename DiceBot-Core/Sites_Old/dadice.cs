    ...
    class dadice: DiceSite
    {
        ...
        void PlaceBetThread(object _High)
        {
            try
            {
                ...
                using (var response = Client.PostAsync("roll", Content))
                {
                    ...
                    tmp =Helpers.json.JsonDeserialize<DADICERollBase>(sEmitResponse2);
                }
                if (tmp.status)
                {
        ...
        protected override bool internalWithdraw(double Amount, string Address)
        {
            try
            {
                ...
                {
                    string sEmitResponse2 = response.Result.Content.ReadAsStringAsync().Result;
                    tmp =Helpers.json.JsonDeserialize<DADICEBlance>(sEmitResponse2);
                }
        ...
        public override void Login(string Username, string Password, string twofa)
        {
            ...
            using (var response = Client.PostAsync("balance", Content))
            {
                ...
                tmp =Helpers.json.JsonDeserialize<DADICEBlance>(Resp);
            }            
            if (tmp.status)
            {
                ...
                {
                    string Resp = response.Result.Content.ReadAsStringAsync().Result;
                    dep =Helpers.json.JsonDeserialize<DADICEBlance>(Resp);
                }
                ...
                DADICEStatsBase tmp2 =Helpers.json.JsonDeserialize<DADICEStatsBase>(sEmitResponse);
    ...
    public class DADICERoll
    {
        ...
        public Bet ToBet()
        {
            Bet tmp = new Bet 
            {
                ...
                Date =Helpers.json.ToDateTime2(timestamp.ToString()),
                ID=id,
