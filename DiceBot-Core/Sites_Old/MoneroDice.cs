...
namespace DiceBotCore.Sites_Old
{
    class MoneroDice: DiceSite
    {
        ...
        void GetBalanceThread()
        {
                    ...
                    try
            {
                    if (priv != "" && pub != "" && (DateTime.Now - lastupdate).TotalSeconds > 60)
                    {
                        ...
                        MoneroStat tmp = Helpers.json.JsonDeserialize<MoneroStat>(sEmitResponse);
                        bets = (int)tmp.bets;
                        wins = (int)tmp.wins;
        ...
        void PlaceBetThread(object high)
        {
            ...
            monerobase tmp = Helpers.json.JsonDeserialize<monerobase>(sEmitResponse);
            if (tmp != null)
            {
                if (tmp.bet_data.error == null)
                {
                    Bet tmp2 = new Bet();
                    tmp2.Amount = Double.Parse(tmp.bet_data.size, System.Globalization.NumberFormatInfo.InvariantInfo);
                    tmp2.Date = DateTime.Now;
                    tmp2.ID = tmp.bet_data.bet_id;
                    tmp2.Profit = (tmp.bet_data.win) ? double.Parse(tmp.bet_data.profit, System.Globalization.NumberFormatInfo.InvariantInfo) : -tmp2.Amount;
                    tmp2.Roll = double.Parse(tmp.bet_data.roll_result, System.Globalization.NumberFormatInfo.InvariantInfo);
                    tmp2.High = tmp.bet_data.roll_type == "over";
                    tmp2.Chance = double.Parse(tmp.bet_data.win_chance, System.Globalization.NumberFormatInfo.InvariantInfo);
