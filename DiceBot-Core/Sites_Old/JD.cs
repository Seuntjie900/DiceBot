...
namespace DiceBotCore.Sites_Old
{
    class JD: DiceSite
    {
        ...
        {
            Bet tmp = new Bet();
            tmp.Amount = double.Parse(curBet.bet, System.Globalization.CultureInfo.InvariantCulture);
            tmp.Date = jdInstance.ToDateTime2( curBet.date.ToString());
            tmp.ID = (long)curBet.betid;
            tmp.Profit = double.Parse(curBet.this_profit, System.Globalization.CultureInfo.InvariantCulture);
            tmp.Roll = (double)curBet.lucky / 10000.0;
            tmp.High = curBet.high;
            tmp.Chance = double.Parse(curBet.chance, System.Globalization.CultureInfo.InvariantCulture);
