    ...
    public class PD : DiceSite
    {
            ...
            try
            {
                while (ispd)
                {
                    if (accesstoken != "" && (DateTime.Now - lastupdate).TotalSeconds > 60)
                    {
                        ...
                        pduser tmpu =Helpers.json.JsonDeserialize<pduser>(sEmitResponse2);
        ...
        public override bool Register(string Username, string Password)
        {
            ...
            try
            {
                ...
                pdlogin tmp =Helpers.json.JsonDeserialize<pdlogin>(sEmitResponse);
                ...
                else
                {
                    ...
                    Content = new FormUrlEncodedContent(pairs);
                     ...
                     tmp =Helpers.json.JsonDeserialize<pdlogin>(sEmitResponse);
                     accesstoken = tmp.access_token;
                    lastupdate = DateTime.Now;
                    return true;
        ...
        void placebetthread()
        {
            try
            {
                ...
                try
                {
                    pdbetresult tmp = Helpers.json.JsonDeserialize<pdbetresult>(sEmitResponse);
        ...
        public override void ResetSeed()
        {
            if ((DateTime.Now - LastSeedReset).TotalSeconds>90)
            {
                try
                {
                    ...
                    PDseeds tmpSeed = Helpers.json.JsonDeserialize<PDseeds>(sEmitResponse);
        ...
        public string getDepositAddress()
        {
            try
            {
                 ...
                 pdDeposit tmpa = Helpers.json.JsonDeserialize<pdDeposit>(sEmitResponse);
                return tmpa.address;
            }
            catch (WebException e)
            {
        ...
        void GetRollThread(object _BetID)
        {
            try
            {
                ...
                pdbet tmp = Helpers.json.JsonDeserialize<pdbet>(sEmitResponse);
                if (tmp.bet.server !="")
                {
                ...
                try
                {
                    if (accesstoken != "")
                    {
                       string sEmitResponse = Client.GetStringAsync("messages?access_token=" + accesstoken + "&room=English").Result;
                        chatmessages msgs =Helpers.json.JsonDeserialize<chatmessages>(sEmitResponse);
                        bool pastlast = false;
                        for (int i = 0; i < msgs.messages.Length; i++)
                        {
                            ...
                            //if (!pastlast)
                            {
                                pastlast =Helpers.json.ToDateTime2(msgs.messages[i].timestamp).Ticks > lastchat.Ticks;
                            }
                            if (pastlast)
                            {
                                lastchat =Helpers.json.ToDateTime2(msgs.messages[i].timestamp);
    ...
    public class pdbet
    {
        ...
        public Bet toBet()
        {
            Bet tmp = new Bet 
            {
                ...
                Date = Helpers.json.ToDateTime2(timestamp),
                ID = long.Parse(id, System.Globalization.CultureInfo.InvariantCulture), 
                Profit=profit/100000000.0, 
                Roll=roll, 
                High=condition==">",
                Chance = condition == ">" ? 99.99 - target : target, 
                Nonce=nonce ,
