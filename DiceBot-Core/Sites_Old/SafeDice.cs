    ...
    class SafeDice : DiceSite
    {
        ...
        protected override void CurrencyChanged()
        {
            ...
            try
            {
                if (accesstoken != "" && accesstoken != null)
                {
                    ...
                    SafeDiceWalletInfo tmp2 = Helpers.json.JsonDeserialize<SafeDiceWalletInfo>(sEmitResponse);
            ...
            while (ispd)
            {
                try
                {
                    ...
                    if (accesstoken != "" && accesstoken != null)
                    {
                        ...
                        SDChat[] sdchat = Helpers.json.JsonDeserialize<SDChat[]>(sEmitResponse);
        ...
        public override void Login(string Username, string Password, string twofa)
        {
            ...
            try
            {
                ...
                SafeDiceLogin tmp = Helpers.json.JsonDeserialize<SafeDiceLogin>(sEmitResponse);
                ...
                else
                {
                    ...
                    SafeDicegetUserInfo tmp1 = Helpers.json.JsonDeserialize<SafeDicegetUserInfo>(sEmitResponse);
        ...
        void PlaceBetThread(object High)
        {
            try
            {
                ...
                string post = Helpers.json.JsonSerializer<SafeDiceBet>(tmpBet);
                ...
                SafeDiceBetResult tmpResult = Helpers.json.JsonDeserialize<SafeDiceBetResult>(Resp);
                Bet bet = new Bet();
                bet.Amount = tmpResult.amount / (curen != 2 ? 100000000.0 : 1000000000000.0);
                bet.Date = Helpers.json.ToDateTime2(tmpResult.processTime);
                bet.Chance = (!tmpResult.isRollLow ? 100.0 - (double)tmpResult.target / 1000000.0 * 100.0 : (double)tmpResult.target / 1000000.0 * 100.0);
                bet.High = !tmpResult.isRollLow;
                bet.ClientSeed = client;
                bet.ID = tmpResult.id;
                bet.Nonce = nonce++;
                bet.Profit = tmpResult.profit / (curen != 2 ? 100000000.0 : 1000000000000.0);
                bet.Roll = tmpResult.roll / 10000.0;
        ...
        public override void ResetSeed()
        {
            try
            {
                ...
                SDRandomize tmp = Helpers.json.JsonDeserialize<SDRandomize>(sEmitResponse);
        ...
        void sendChatThread(object Message)
        {
            if (accesstoken != "" && accesstoken != null)
            {
                try
                {
                    ...
                    HttpWebResponse EmitResponse = (HttpWebResponse)loginrequest.GetResponse();*/
                    string post = Helpers.json.JsonSerializer<SDSendChat>(new SDSendChat { _message = (string)Message, room = "en_US" });
                    HttpContent cont = new StringContent(post);
                    cont.Headers.ContentType = new System.Net.Http.Headers.MediaTypeHeaderValue("application/json");
                    string Resp = "";
        ...
        protected override bool internalWithdraw(double Amount, string Address)
        {
            try
            {
                ...
                HttpWebResponse EmitResponse = (HttpWebResponse)loginrequest.GetResponse();*/
                string post = Helpers.json.JsonSerializer<SDSendWIthdraw>(new SDSendWIthdraw { amount = (long)(Amount * 100000000), address = Address });
                HttpContent cont = new StringContent(post);
                cont.Headers.ContentType = new System.Net.Http.Headers.MediaTypeHeaderValue("application/json");
                string Resp = "";
        ...
        public override bool Register(string username, string password)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    loginrequest.CookieContainer.Add(new Cookie("token", accesstoken, "/", "safedice.com"));
                    loginrequest.Headers.Add("Authorization", "Bearer " + accesstoken);
                    post = Helpers.json.JsonSerializer<SDSetPw>(new SDSetPw { username = username, password = password });
                    loginrequest.ContentLength = post.Length;
                    using (var writer = new StreamWriter(loginrequest.GetRequestStream()))
                    {
        ...
        public override bool Invest(double Amount)
        {
            try
            {
                ...
                HttpWebResponse EmitResponse = (HttpWebResponse)loginrequest.GetResponse();*/
                string post = Helpers.json.JsonSerializer<SDSendInvest>(new SDSendInvest { amount = (long)(Amount * 100000000) });
                HttpContent cont = new StringContent(post);
                cont.Headers.ContentType = new System.Net.Http.Headers.MediaTypeHeaderValue("application/json");
                string Resp = "";
        ...
        string GetDepositAddress()
        {
            ...
            try
            {
                ...
                return Helpers.json.JsonDeserialize<SDDEpost>(sEmitResponse).address;
            }
            catch (AggregateException e)
            {
                //Parent.DumpLog(e.InnerException.Message, 3);
