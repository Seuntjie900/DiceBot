...
namespace DiceBotCore.Sites_Old
{
    class CoinMillions : DiceSite
    {
                    ...
                    if (accesstoken != "")
                    {
                        if ((DateTime.Now - lastupdate).TotalSeconds > 60)
                            ...
                            try
                            {
                                ...
                                cmRankings tmpu =Helpers.json.JsonDeserialize<cmRankings>(s);
                        ...
                        if ((DateTime.Now - lastChat).TotalSeconds > 1)
                        {
                            try
                            {
                                ...
                                if (lastChatId == 0)
                                {
                                    ...
                                    CmChatItem[] chats =Helpers.json.JsonDeserialize<CmChatItem[]>(s);
                                    foreach (CmChatItem c in chats)
                                    {
                                        ReceivedChatMessage(string.Format("{0} ({1}) <{2}> {3}",Helpers.json.ToDateTime2(c.created.ToString()), c.user_id, c.username, c.content));
        ...
        void PlaceBetThread(object _High)
        {
            ...
            try
            {
                ...
                CmBetResult tmp =Helpers.json.JsonDeserialize<CmBetResult>(responseData);
    ...
    public class CmBetResult
    {
        ...
        {
            Bet tmp = new Bet {
                Amount = double.Parse(amount, System.Globalization.NumberFormatInfo.InvariantInfo),
                Date = DateTime.Now,
                ID = id,
                Profit = double.Parse(profit, System.Globalization.NumberFormatInfo.InvariantInfo),
                High = roll_type == "h",
                Chance = (double)chance,
                Roll = double.Parse(lucky_number, System.Globalization.NumberFormatInfo.InvariantInfo)
